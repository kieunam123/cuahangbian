...
/**
 * Class Almaira_Shop_Repeater
 */
class Almaira_Shop_Repeater extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->add_field_label = esc_html__( 'Add new field', 'almaira-shop' );
		if ( ! empty( $args['add_field_label'] ) ) {
			$this->add_field_label = $args['add_field_label'];
		}

		$this->boxtitle = esc_html__( 'Customizer Repeater', 'almaira-shop' );
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'almaira_customizer-repeater-admin-stylesheet', get_template_directory_uri() . '/customizer/repeater/style.css', array(), '1.0.0' );

		wp_enqueue_style( 'wp-color-picker' );

		wp_enqueue_script( 'almaira_customizer-repeater-script', get_template_directory_uri() . '/customizer/repeater/script.js', array( 'jquery', 'jquery-ui-draggable', 'wp-color-picker' ), '1.0.0', true );

		wp_enqueue_script( 'almaira_customizer-repeater-fontawesome-iconpicker', get_template_directory_uri() . '/customizer/repeater/icon-picker/icon-picker.js', array( 'jquery' ), '1.0.0', true );

		wp_enqueue_style( 'almaira_customizer-repeater-fontawesome-iconpicker-script', get_template_directory_uri() . '/customizer/repeater/icon-picker/icon-picker.css', array(), '1.0.0' );
	}
			...
			foreach ( $array as $icon ) {
				...
				<div class="customizer-repeater-general-control-repeater-container customizer-repeater-draggable">
					...
					<div class="customizer-repeater-box-content-hidden">
						<?php
						$choice = '';
						if ( $this->id === 'almaira_features_content' ) {
						...
						if ( $this->customizer_repeater_color_control == true ) {
							$this->input_control(
								array(
									'label'             => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Color', 'almaira-shop' ), $this->id, 'customizer_repeater_color_control' ),
									'class'             => 'customizer-repeater-color-control',
									'type'              => apply_filters( 'almaira_repeater_input_types_filter', 'color', $this->id, 'customizer_repeater_color_control' ),
						...
						if ( $this->customizer_repeater_color2_control == true ) {
							$this->input_control(
								array(
									'label'             => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Color', 'almaira-shop' ), $this->id, 'customizer_repeater_color2_control' ),
									'class'             => 'customizer-repeater-color2-control',
									'type'              => apply_filters( 'almaira_repeater_input_types_filter', 'color', $this->id, 'customizer_repeater_color2_control' ),
									'sanitize_callback' => 'sanitize_hex_color',
								), $color2
							);
						}
						if ( $this->customizer_repeater_title_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Title', 'almaira-shop' ), $this->id, 'customizer_repeater_title_control' ),
									'class' => 'customizer-repeater-title-control',
									'type'  => apply_filters( 'almaira_repeater_input_types_filter', '', $this->id, 'customizer_repeater_title_control' ),
								), $title
							);
						}
...
///////////// price///////////
						if ( $this->customizer_repeater_price_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Price', 'almaira-shop' ), $this->id, 'customizer_repeater_price_control' ),
									'class' => 'customizer-repeater-price-control',
									'type'  => apply_filters( 'almaira_repeater_input_types_filter', '', $this->id, 'customizer_repeater_price_control' ),
								), $price
							);
						}
...
///////////// number///////////
						if ( $this->customizer_repeater_number_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Number', 'almaira-shop' ), $this->id, 'customizer_repeater_number_control' ),
									'class' => 'customizer-repeater-number-control',
									'type'  => apply_filters( 'almaira_repeater_input_types_filter', '', $this->id, 'customizer_repeater_number_control' ),
								), $number
							);
						}
						...
						if ( $this->customizer_repeater_subtitle_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Subtitle', 'almaira-shop' ), $this->id, 'customizer_repeater_subtitle_control' ),
									'class' => 'customizer-repeater-subtitle-control',
									'type'  => apply_filters( 'almaira_repeater_input_types_filter', '', $this->id, 'customizer_repeater_subtitle_control' ),
								), $subtitle
							);
						}
						if ( $this->customizer_repeater_text_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Text', 'almaira-shop' ), $this->id, 'customizer_repeater_text_control' ),
									'class' => 'customizer-repeater-text-control',
									'type'  => apply_filters( 'almaira_repeater_input_types_filter', 'textarea', $this->id, 'customizer_repeater_text_control' ),
								), $text
							);
						}
						if ( $this->customizer_repeater_link_control ) {
							$this->input_control(
								array(
									'label'             => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Link', 'almaira-shop' ), $this->id, 'customizer_repeater_link_control' ),
									'class'             => 'customizer-repeater-link-control',
									'sanitize_callback' => 'esc_url_raw',
									'type'              => apply_filters( 'almaira_repeater_input_types_filter', '', $this->id, 'customizer_repeater_link_control' ),
								), $link
							);
						}
						if ( $this->customizer_repeater_text2_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Button Text', 'almaira-shop' ), $this->id, 'customizer_repeater_text2_control' ),
									'class' => 'customizer-repeater-text2-control',
									'type'  => apply_filters( 'almaira_repeater_input_types_filter', 'textarea', $this->id, 'customizer_repeater_text2_control' ),
								), $text2
							);
						}
						if ( $this->customizer_repeater_link2_control ) {
							$this->input_control(
								array(
									'label'             => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Link', 'almaira-shop' ), $this->id, 'customizer_repeater_link2_control' ),
									'class'             => 'customizer-repeater-link2-control',
									'sanitize_callback' => 'esc_url_raw',
									'type'              => apply_filters( 'almaira_repeater_input_types_filter', '', $this->id, 'customizer_repeater_link2_control' ),
								), $link2
							);
						}
						if ( $this->customizer_repeater_shortcode_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Shortcode', 'almaira-shop' ), $this->id, 'customizer_repeater_shortcode_control' ),
									'class' => 'customizer-repeater-shortcode-control',
									'type'  => apply_filters( 'almaira_repeater_input_types_filter', '', $this->id, 'customizer_repeater_shortcode_control' ),
								), $shortcode
							);
						}
						...
						echo '>';
						esc_html_e( 'Delete field', 'almaira-shop' );
						?>
						</button>
...
//number control//////////
					...
					<button type="button" class="social-repeater-general-control-remove-field button" style="display:none;">
						<?php esc_html_e( 'Delete field', 'almaira-shop' ); ?>
					</button>
				</div>
			</div>
			<?php
	...
	private function input_control( $options, $value = '' ) {
		...
		if ( ! empty( $options['type'] ) ) {
			switch ( $options['type'] ) {
				...
				case 'color':
					$style_to_add = '';
					if ( $this->id === 'almaira_features_content' && $options['choice'] !== 'customizer_repeater_icon' ) {
	...
	private function icon_picker_control( $value = '', $show = '' ) {
		...
		?>
			>
			<span class="customize-control-title">
				<?php esc_html_e( 'Icon', 'almaira-shop' ); ?>
			</span>
			<span class="description customize-control-description">
				<?php
				echo sprintf(
					/* translators: Fontawesome link with full list of icons available */
					esc_html__( 'Note: Some icons may not be displayed here. You can see the full list of icons at Fontawesome Website.', 'almaira-shop' )
	...
	private function image_control( $value = '', $show = '' ) {
		...
		<?php
		if ( $show === 'customizer_repeater_icon' || $show === 'customizer_repeater_none' || ( $this->id === 'almaira_features_content' && empty( $show ) ) ) {
			echo 'style="display:none;"'; }
		?>
			>
			<span class="customize-control-title">
				<?php esc_html_e( 'Image', 'almaira-shop' ); ?>
			</span>
			<input type="text" class="widefat custom-media-url" value="<?php echo esc_attr( $value ); ?>">
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e( 'Upload Image', 'almaira-shop' ); ?>" />
		</div>
		<?php
	}
	...
	private function icon_type_choice( $value = 'customizer_repeater_icon' ) {
		?>
		<span class="customize-control-title">
			<?php esc_html_e( 'Image type', 'almaira-shop' ); ?>
		</span>
		<select class="customizer-repeater-image-choice">
			<option value="customizer_repeater_icon" <?php selected( $value, 'customizer_repeater_icon' ); ?>><?php esc_html_e( 'Icon', 'almaira-shop' ); ?></option>
			<option value="customizer_repeater_image" <?php selected( $value, 'customizer_repeater_image' ); ?>><?php esc_html_e( 'Image', 'almaira-shop' ); ?></option>
			<option value="customizer_repeater_none" <?php selected( $value, 'customizer_repeater_none' ); ?>><?php esc_html_e( 'None', 'almaira-shop' ); ?></option>
		</select>
		<?php
	}
	...
	private function repeater_control( $value = '' ) {
		...
		$show_del        = 0;
		?>
		<span class="customize-control-title"><?php esc_html_e( 'Social icons', 'almaira-shop' ); ?></span>
		<?php
		...
		if ( ( count( $social_repeater ) == 1 && '' === $social_repeater[0] ) || empty( $social_repeater ) ) {
			...
			<div class="customizer-repeater-social-repeater">
				<div class="customizer-repeater-social-repeater-container">
					...
					<?php get_template_part( $this->customizer_icon_container ); ?>
					<input type="text" class="customizer-repeater-social-repeater-link" placeholder="<?php esc_attr_e( 'Link', 'almaira-shop' ); ?>">
					<input type="hidden" class="customizer-repeater-social-repeater-id" value="">
					<button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e( 'Remove Icon', 'almaira-shop' ); ?>
					</button>
				</div>
				<input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
			</div>
			<button class="social-repeater-add-social-item button-secondary"><?php esc_html_e( 'Add Icon', 'almaira-shop' ); ?></button>
			<?php
		} else {
			?>
			<div class="customizer-repeater-social-repeater">
				...
				foreach ( $social_repeater as $social_icon ) {
					...
					if ( ! empty( $social_icon['icon'] ) ) {
						...
						echo '<input type="text" class="customizer-repeater-social-repeater-link" placeholder="' . esc_attr__( 'Link', 'almaira-shop' ) . '" value="';
					...
					if ( $show_del == 1 ) {
						...
						echo '">' . esc_html__( 'Remove Icon', 'almaira-shop' ) . '</button>';
					echo '</div>';
				}
				?>
